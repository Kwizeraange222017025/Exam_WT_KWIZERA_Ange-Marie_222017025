...
<label for="enroll_date">enroll_date:</label>
<input type="date" id="enroll_date" name="enroll_date"/><br><br>


<input type="submit" name="enrollment" value="INSERT"/><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO enrollment(enrollment_id, user_id, course_id, enroll_date) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $enid, $usid, $cid, $ed);
    // Set parameters and execute
    $enid = $_POST['enrollment_id'];
    $usid = $_POST['user_id'];
    $cid = $_POST['course_id'];
    $ed = $_POST['enroll_date'];
...
include('database_connection.php');
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $enid = $row['enrollment_id']; // Fetch the enrollment_id
                echo "<tr>
                    <td>" . $row['enrollment_id'] . "</td>
                    <td>" . $row['user_id'] . "</td>
                    <td>" . $row['course_id'] . "</td>
                    <td>" . $row['enroll_date'] . "</td>
                    <td><a style='padding:4px' href='deleteenrollment.php?enrollment_id=$enid'>Delete</a></td> 
                    <td><a style='padding:4px' href='updateenrollment.php?enrollment_id=$enid'>Update</a></td> 
                </tr>";
            }
        } else {
