...
<input type="number" id="post_id" name="post_id"/><br><br>
...
<input type="submit" name="post" value="INSERT"/><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO post(post_id, forum_id, user_id, content) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $pid, $fid, $uid, $cont);
...
// Connection details
include('database_connection.php');
// SQL query to fetch data from the post table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of post</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of post</h2></center>
    <table border="5">
        <tr>
            <th>post_id </th>
            <th>forum_id </th>
...
include('database_connection.php');
        ...
        // Prepare SQL query to retrieve all post
        $sql = "SELECT * FROM post";
        $result = $connection->query($sql);

        // Check if there are any post
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $pid = $row['post_id']; // Fetch the post_id
                echo "<tr>
                    <td>" . $row['post_id'] . "</td>
                    <td>" . $row['forum_id'] . "</td>
                    <td>" . $row['user_id'] . "</td>
                    <td>" . $row['content'] . "</td>
                    <td><a style='padding:4px' href='deletepost.php?post_id=$pid'>Delete</a></td> 
                    <td><a style='padding:4px' href='updatepost.php?post_id=$pid'>Update</a></td> 
                </tr>";
            }
        } else {
